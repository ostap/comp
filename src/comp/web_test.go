...
func ExampleComps() {
	...
	run(`[ i["\"a\""] | i <- [{"a"}, {"b"}, {"c"}]]`)
	run(`[ i | i <~ [1,2,3,4], $i == 2 ]`)
	run(`[ {$i,i} | i <- ["1a","2b","3c","4d","5e","6f"], $i < 3 ]`)
	run(`[{$i, i, $j, j, $k, k}|i<-["a","b","c"],j <- [-1, -2],k <- [{id:0},{id:1}], $i < 1]`)
	...
	// ["a","b","c"]
	// [3]
	// [{"$i":0,"i":"1a"},{"$i":1,"i":"2b"},{"$i":2,"i":"3c"}]
	// [{"$i":0,"$j":0,"$k":0,"i":"a","j":-1,"k":{"id":0}},{"$i":0,"$j":0,"$k":1,"i":"a","j":-1,"k":{"id":1}},{"$i":0,"$j":1,"$k":0,"i":"a","j":-2,"k":{"id":0}},{"$i":0,"$j":1,"$k":1,"i":"a","j":-2,"k":{"id":1}}]
}
